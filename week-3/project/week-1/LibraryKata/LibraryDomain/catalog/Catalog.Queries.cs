...
// I don't have to mirrow the interface implementation or any inheritance across both class files
// however, I can still only inherit from the parent
public partial class Catalog : IEnumerable<LibraryItem>
{
    ...
    public IEnumerator<LibraryItem> GetEnumerator()
    {
        ...
        {
            // We wamt to lazily return items on at a time, we don't want to return a second list
    ...
    // We are going to use Predicate to pass a delegate to our fucntion
    // A delegate is just a reference to method in an argument list
    // Predicate<LibraryItem> match represents a function that takes a library item an returns a boolean

    // when we call this find() we will combine it with a lambda. Lambdas are the c# implementation 
    // of anonymous or arrow functions. Just a quick definition that we don't bother staring a reference to.

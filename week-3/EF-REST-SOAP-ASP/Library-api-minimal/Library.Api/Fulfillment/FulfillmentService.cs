...
public class FulfillmentService : IFulfillmentService
{
    ...
    public async Task<FulfillmentResult> FulfillOneAsync(int orderId, CancellationToken ct)
    {
        ...
        // lets create that dictionary with the productId
        ...
        if (!canFulfill) // checking for canFulfill == false
        {
            ...
            // Create a new fulfillment event record for this transaction, 
        ...
        db.FulfillmentEvents.Add(new FulfillmentEvent {OrderId = orderId, Type = "Fullfilled"});

        // Adding our retry save method
        if(!await SaveWithRetryAsync(db, requested, ct)) // if we need this if - we lost enough times
        {
            db.ChangeTracker.Clear();
            Order staleOrder = await db.Orders.FirstAsync(o => o.Id == orderId, ct); // grab stale order fro mdb
        ...
        LibraryDbContext db, IReadOnlyDictionary<int, int> requestedByProductId, CancellationToken ct)
    {
        ...
        while(true)
        {
            ...
            try
            {
                ...
                // staged to it - we can save them here. It's the same object
                await db.SaveChangesAsync(ct);
                return true;
            }

...
Log.Logger = new LoggerConfiguration()
    .WriteTo.Console()
    .WriteTo.File("logs/fulfillment-log-log-.log", rollingInterval: RollingInterval.Day)
    .CreateLogger();
...
// Transient - a new instance is created everytime it's requested
...
// We know we will need more than one LibraryDbContext in one of these methods. But we don't know how many 
...
app.MapGet("/peek/conflict", (IServiceScopeFactory scopes) =>
{
    ...
    // Now remember that dbContext is generated per scope, so we have to do that too
    ...
    // remember we gave Inventory entity a RowVersion - not just a property named RowVersion
    ...
    firstDb.SaveChanges(); // Save changes is what persists any craetion, deletion or modified objects

    // Calling SaveChanges() above 
    ...
    try
    {
        secondDb.SaveChanges(); // This should fail as RowVersoins don't match
...
app.MapPost("/inventory/rest", (LibraryDbContext db, ILogger<Program> logger) =>
{
    ...
    {
        // I only want to do something if the primary key is 1, 2, 3...
        switch (inv.Id)
        {
            case 1:
                inv.CurrentStock = 5;
...
// Quick method to fulfillm one order
            ...
            CancellationToken ct, IFulfillmentService fSvc) =>
{
    ...
    // And then try to create a Successful fulfulment record against the db
